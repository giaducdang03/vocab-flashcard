...
**Route ordering:** React Router v6 ranks static segments above dynamic ones automatically, so `/sessions` before `/sessions/:id` is handled for you. Just keep them adjacent for readability.

## Implementation

### Step 1: Create SessionsPage component

Create new file `frontend/src/pages/SessionsPage.tsx`:
...
Add this `<Route>` immediately before the existing `/sessions/:id` route (should be around line 1485 in the existing file):
...
In the browser:
- Visit `/sessions` — page loads with PageHeader, title "All sessions", search box, grid of cards
- Header "Sessions" pill is active at `/sessions`
- "Sessions" pill is NOT active at `/`
- Click `/sessions/:id` from a card — detail page still opens (static route does not swallow dynamic)
- Footer "Sessions" link works
